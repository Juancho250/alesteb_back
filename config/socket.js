...
const { notifyUser, Payloads } = require('../src/modules/notifications').push;
...
const initSocket = (httpServer) => {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('chat:dm', async ({ recipientId, message }) => {
      ...
      if (!Number.isSafeInteger(parsedRecipientId) || parsedRecipientId <= 0 || !cleanMessage || cleanMessage.length > 2000) {
        return socket.emit('chat:error', { code: 'DM_INVALID_INPUT', message: 'Mensaje invalido' });
      }
      try {
        ...
        if (!recipientCheck.rows.length) {
          return socket.emit('chat:error', { code: 'DM_RECIPIENT_NOT_FOUND', message: 'Destinatario no disponible' });
        }
