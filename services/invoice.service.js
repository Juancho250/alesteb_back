...
async function generateInvoicePdf(params) {
  ...
  // ── Branding defaults ──────────────────────────────────────────────────────
  const bizName    = branding?.businessName  || 'Alesteb Boutique';
  const bizEmail   = branding?.businessEmail || 'web@alesteb.com';
  ...
  return new Promise((resolve, reject) => {
    ...
    doc.fillColor('#94a3b8').fontSize(8).font('Helvetica')
       .text(
         `© ${now.getFullYear()} ${bizName} · Gestionado con Alesteb ERP · ${bizEmail}`,
