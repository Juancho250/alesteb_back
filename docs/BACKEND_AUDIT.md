...
`alesteb_back` es una API REST multi-tenant para la gestión de boutiques (tiendas de moda colombianas). Cada tienda es un "admin" independiente; sus operadores (gerente, cajero…) son sub-usuarios que heredan el scope del admin dueño. Un superadmin especial tiene visibilidad total sobre todos los tenants.
...
| **Descripción** | `SENDER.email` cae a `softturin@gmail.com` si `BREVO_SENDER_EMAIL` no está configurado. En producción con múltiples tiendas, todos los emails transaccionales salen del mismo remitente de la plataforma. |
